...
class OOOP:
    ...
    def __init__(self, user='admin', pwd='admin', dbname='odoo',
                 ...
                 exe=False, active=True, readonly=False, fast=False, **kwargs):
        ...
        self.connect()
        # Remenber to use add_model() in fast mode
        if not fast:
            try:
                self.load_models()
    ...
    def connect(self):
        ...
        self.objectsock = xmlrpclib.ServerProxy('%s:%i/xmlrpc/object' % (self.uri, self.port), allow_none=self.exe)
    ...
    def __set_model(self, model, r={}, deep=None):
